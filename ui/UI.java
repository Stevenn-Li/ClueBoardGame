...
//created labels
		...
		_accusation = new JButton("Make Accusation");
		JLabel _textbox = new JLabel("Make accusation below here:");
		JLabel _textbox2 = new JLabel("Make suggestion below here:");
		buttonPanel.add(_suggestion);
		buttonPanel.add(_rollDice);
		buttonPanel.add(_skip);
		buttonPanel.add(_accusation);
		...
		_accusation.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
			//here is where we're going to implement the new pane	
			_accusationInput.grabAccusationInput();	
		...
		_message.setLocation(WINDOW_HEIGHT - PADDING * 2 + PADDING, WINDOW_HEIGHT - BUTTON_HEIGHT - LABEL_HEIGHT - PADDING * 7 - INPUT_HEIGHT);
	...
	//accusation input box, button, sets text for accusation and suggestion
		_accusationInput = new AccusePane(this);
		_accusationInput.setSize(WINDOW_WIDTH - (WINDOW_HEIGHT - PADDING * 2) - PADDING * 2, INPUT_HEIGHT);
		_accusationInput.setLocation(WINDOW_HEIGHT - PADDING * 2 + PADDING, WINDOW_HEIGHT - BUTTON_HEIGHT - PADDING * 10 - INPUT_HEIGHT * 2);
		_mainPanel.add(_accusationInput);
		_textbox.setSize(WINDOW_WIDTH - (WINDOW_HEIGHT - PADDING * 2) - PADDING * 2, INPUT_HEIGHT);
		_textbox.setLocation(WINDOW_HEIGHT - PADDING * 2 + PADDING, WINDOW_HEIGHT - BUTTON_HEIGHT - PADDING * 17 - INPUT_HEIGHT * 2);
		_textbox2.setSize(WINDOW_WIDTH - (WINDOW_HEIGHT - PADDING * 2) - PADDING * 2, INPUT_HEIGHT);
		_textbox2.setLocation(WINDOW_HEIGHT - PADDING * 2 + PADDING, WINDOW_HEIGHT - BUTTON_HEIGHT - PADDING * 10 - INPUT_HEIGHT);	
		_mainPanel.add(_textbox);
		_mainPanel.add(_textbox2);
		update();
	...
	/**
	 * Basically, you make an accusation and here is where it's checked
	 * If your accusation is correct, then you win
	 * If you accusation is wrong, you lose, which basically means
	 * you get skipped every turn, though your cards remain.
	 * 
	 * Losing basically means that your loser status is true,
	 * sort of like running with your arms behind you to go faster
	 * @param accusation
	 * 
	 * @author Noah Poczciwinski wrote the passSuggestion method that this is based off of
	 * @author Joseph Soares 
	 */
	public void passAccusation(ArrayList<String> accusation){
		boolean answer = _game.makeAccusation(accusation);
		if (answer == true){
			displayMessage("you win!!");
		}else{
			displayMessage("you lose!!");
			_game.getCurrentPlayer().setLoser();
			_game.skip();
		}
	}
	/**
