...
public class Game {
	...
	private ArrayList<Card> _possibleWeapons = new ArrayList<Card>();
	private ArrayList<Card> _possiblePlayers = new ArrayList<Card>();
	private ArrayList<Card> _possibleRooms = new ArrayList<Card>();
	...
	public Game(int numPlayers) {
		...
		System.out.println("Case File Confidential:" + _envelope);
	}
	...
	private void readGameInfo() {
		...
		try {
			...
			//legal cards that are playable.
			_possiblePlayers.addAll(players);
			_possibleWeapons.addAll(weapons);
			_possibleRooms.addAll(rooms);
	...
	public Player getCurrentPlayer(){
		return _players.get(_currentTurn);
	}
	...
	/**
	 * Simple method to skip over people who have lost the game.
	 * 
	 * Checks user's "loser" status to determine if he/she is cool
	 * enough to play the game
	 * 
	 * @author Joseph Soares
	 */
	
	public void skipLosers(){
		if(_players.get(_currentTurn).loserStatus() == true){
			skip();
		}
	}
	public ArrayList<Card> getPossibleWeapons(){
		return _possibleWeapons;
	}
	
	public ArrayList<Card> getPossiblePlayers(){
		return _possiblePlayers;
	}
	
	public ArrayList<Card> getPossibleRoom(){
...
public boolean makeAccusation(ArrayList<String> accusation) {
			if (accusation.contains(_envelope.get(0).getName()) &&
					accusation.contains(_envelope.get(1).getName()) &&
					accusation.contains(_envelope.get(2).getName())
			){
				return true;
		}	
			return false;
